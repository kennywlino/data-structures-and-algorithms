...
const longestString = (arr) => {
  ...
  let longestStringIdx = 0;
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].length > arr[longestStringIdx].length) {
      longestStringIdx = i;
    }
  }
...
const unenrollBrook = (arr) => {
  return arr.map(innerArr => innerArr.filter(element => !element.includes('Brook')));
};
  ...
  arr.forEach(activity => {
    daysOfWeek.forEach((day, index) => {
      if (activity.includes(day)) {
        activitiesByDay[index].push(activity);
      }
    });
  });
