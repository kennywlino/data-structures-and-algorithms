...
const addPurchases = (arr) => {
  /* return arr.reduce((accumulator, value) => {
    accumulator = accumulator.purchasePrice + value.purchasePrice;
    return accumulator;
  }, 0); */
...
const calculateAverage = (arr) => {
  let countAndSumInfo = arr.reduce((accumulator, value) => {
    accumulator.count = accumulator.count + 1;
    accumulator.sum = accumulator.sum + value;
    return accumulator;
  }, { count: 0, sum: 0 });
  return countAndSumInfo.sum / countAndSumInfo.count;
};
...
const extractChildren = (arr) => {
  let filteredArr = arr.filter(element => element.name.toLowerCase().includes('a'));
  return filteredArr.reduce((accumulator, value) => {
    if(value.children) {
      accumulator = [...accumulator, ...value.children];
    }
    return accumulator;
  }, []);
};
