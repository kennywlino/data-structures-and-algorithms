...
const getStatName = (arr, minBaseStat) => {
  let filtered = arr.filter(element => element.baseStat > minBaseStat);
  return filtered.map(element => element.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  let numsOnly = arr.filter(element => typeof element === 'number');
  return numsOnly.map(element => (element % 2 === 0) ? "even" : "odd");
};
