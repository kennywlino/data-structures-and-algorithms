...
// used matchAll() which allows us to access the capture groups as opposed to match()
// matchAll() also returns an Iterator which needs to be opened with [...]
// the resulting structure would be [[match0, captureGroup0] [match1, captureGroup1]]
...
const findTagNames = elements => {
  ...
  elements.forEach(element => {
    let matches = [...element.matchAll(regex)];
    matches = matches.map(arr => arr[1]); // only the capture group
    closingTags = [...closingTags, ...matches];
  });
  return closingTags;
};
