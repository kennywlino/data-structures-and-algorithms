...
const count = (target, input) => {
  return input.flat().filter(element => element === target).length;
};
...
const totalSum = (input) => {
  return input.flat().reduce((accumulator, value) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  let inputFiltered = input.map(innerArr => innerArr.filter(element => typeof element === 'number' && element % 5 === 0));
  return inputFiltered.map(innerArr => innerArr.map(element => Math.pow(2, element)));
};
...
let findMaleAndFemale = (data) => {
  let maleAndFemale = data.filter(element => element.gender === 'male' || element.gender === 'female');
  return maleAndFemale.reduce((accumulator, value, index) => {
    if (index < maleAndFemale.length - 1) {
...
let findShortest = (data) => {
  let shortestObj = data.reduce((accumulator, value) => {
    if (Number(value.height) < Number(accumulator.height)) {
      accumulator = value;
    }
    return accumulator;
  });
