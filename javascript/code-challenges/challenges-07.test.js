...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    // we need to slice on the 1st space and the 2nd space
    // everything after the 2nd space is our ingredients
    let spaceIdx1 = element.indexOf(' ');
    let ingredientOnly = element.slice(spaceIdx1 + 1, element.length);
    let spaceIdx2 = ingredientOnly.indexOf(' ');
    ingredientOnly = ingredientOnly.slice(spaceIdx2 + 1, ingredientOnly.length);
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(element => {
    let ingredientSplit = element.split(' ');
    let ingredientOnly = ingredientSplit.slice(2, ingredientSplit.length);
    ingredientOnly = ingredientOnly.join(' ');
    result.push(ingredientOnly);
  });
  return result;
};
...
const stepActions = (recipe) => {
  return recipe.steps.map(element => element.split(' ')[0]);
};
...
const removeEvenValues = (arr) => {
  for (let i = 0; i < arr.length; i++) {
    if(arr[i] % 2 === 0) {
      arr.splice(i, 1);
      i--; // because the elements move down one when there's a deletion
    }
  }
  return arr;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  return numberOfCharacters >= 0 ? str.slice(0, str.length - numberOfCharacters) : str;
};
...
const removeVowels = (str) => {
  ...
  let filteredStrArr = strArr.filter(element => !vowels.includes(element));
  return filteredStrArr.join('');
