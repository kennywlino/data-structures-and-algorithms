...
const updateAnimal = (arr, callback) => {
  let updatedArr = [];
  arr.forEach(element => updatedArr.push(callback(element)));
  return updatedArr;
};
...
const sortNames = (arr) => {
  return arr.sort((nameA, nameB) => nameA < nameB ? -1 : 1);
};
...
const alphabetizeBetter = (arr) => {
  return arr.sort((wordA, wordB) => wordA.toLowerCase() < wordB.toLowerCase() ? -1 : 1);
};
...
const sortPeople = (arr) => {
  return arr.sort((personA, personB) => personA.lastName < personB.lastName ? -1 : 1);
};
...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((a,b) => daysOfWeek.indexOf(a.dayOfWeek) - daysOfWeek.indexOf(b.dayOfWeek));
};
