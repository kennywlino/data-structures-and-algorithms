...
const hasChildrenEntries = (arr, character) => {
  let charObj = arr.find(obj => obj.name === character);
  let charObjEntries = Object.entries(charObj);
  return charObjEntries.some(element => element[1] instanceof Array);
};
...
const houseSize = (arr) => {
  ...
  for (let character of arr) {
    let houseObj = { house: character.house };
    let membersCount = 1; // for character.name
    if (character.spouse) {
      membersCount = membersCount + 1;
    }
    if (character.children) {
      membersCount = membersCount + character.children.length;
    }
    houseObj['members'] = membersCount;
    sizes.push(houseObj);
  }
